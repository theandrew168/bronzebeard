...
class ELF:
    ...
    def build(self):
        ...
        elf.extend(b'\x00\x00\x40\x00\x00\x00\x00\x00')  # TODO entry point: 0x400000 + offset to _start?
        elf.extend(b'\x40\x00\x00\x00\x00\x00\x00\x00')  # TODO program header table offset: 0x40 usually?
        elf.extend(b'\x80\x00\x00\x00\x00\x00\x00\x00')  # TODO section header table offset: 0x80 not sure?
        elf.extend(b'\x00\x00\x00\x00')  # flags: 0
        elf.extend(b'\x40\x00')  # header size: 64 bytes
        elf.extend(b'\x40\x00')  # TODO program header entry size: 64 bytes?
        elf.extend(b'\x01\x00')  # TODO program header entries: depends?
        elf.extend(b'\x40\x00')  # TODO section header entry size: 64 bytes?
        elf.extend(b'\x04\x00')  # TODO section header entries: depends?
        elf.extend(b'\x03\x00')  # TODO: index of names in section header entry: 3?
